...
# %% auto 0
__all__ = ['ResizeMax', 'PadSquare', 'CustomTrivialAugmentWide', 'CustomRandomIoUCrop', 'calculate_iou_multi', 'rand_rect_copy',
           'RandRectCopy', 'rand_pixel_copy', 'RandPixelCopy']
...
def calculate_iou_multi(boxes1:torch.Tensor, # An array of bounding boxes in [x1, y1, x2, y2] format.
...
def rand_rect_copy(img_tensor:torch.Tensor, # The input image tensor.
                   pct:float # The percentage of the image tensor's size to be used as the side length of the rectangle regions.
                  ) -> (torch.Tensor, Tuple): # The modified input image tensor and Coordinates of the target patch in the format [x, y, w, h].
    """
    Copy data from a randomly selected rectangular region to another randomly selected rectangular region of an image tensor, 
    and return the coordinates of the target patch.
    """
    ...
    img_tensor[..., stx2:stx2 + szx, sty2:sty2 + szy] = img_tensor[..., stx1:stx1 + szx, sty1:sty1 + szy]
    
    # The coordinates of the target patch
    target_patch_coords = [sty2, stx2, sty2+szy, stx2+szx]
    
    # Return the modified input img_tensor and the coordinates
    return img_tensor, target_patch_coords

# %% ../nbs/00_core.ipynb 26
class RandRectCopy(transforms.Transform):
    """
    A PyTorch module that applies the `rand_rect_copy` function to an input tensor multiple times.
    """
    def __init__(self, 
    ...
    def _(self, inpt: Image.Image, params: Dict[str, Any]) -> Any:
        ...
        for i in range(random.randint(self.min_num, self.max_num)):
            inpt_torch, patch = rand_rect_copy(inpt_torch, max(0.1,random.random()*self.pct))
            self.patches.append(patch)
            
        return transforms.ToPILImage()(inpt_torch)
    
    @_transform.register
    def _(self, inpt: BoundingBoxes, params: Dict[str, Any]) -> Any:
        
        if len(self.patches) > 0:
            iou_matrix = calculate_iou_multi(inpt, torch.tensor(self.patches, dtype=torch.float32))
...
def rand_pixel_copy(img_tensor:torch.Tensor, # The input image tensor.
                    pct:float=0.2 # The percentage of the total number of pixels to be selected as the source and target sets of pixels.
...
class RandPixelCopy(transforms.Transform):
    """
    A PyTorch module that applies the `rand_pixel_copy` function to an input tensor.
    """
    def __init__(self, 
                 pct:float=0.1 # The maximum percentage of the tensor's pixels to be copied.
                ):
        super().__init__()
        self.pct = pct
    ...
    def _(self, inpt: Image.Image, params: Dict[str, Any]) -> Any:
        ...
        inpt_torch = transforms.PILToTensor()(inpt)
        return transforms.ToPILImage()(rand_pixel_copy(inpt_torch, max(0.0025, random.random() * self.pct)))
